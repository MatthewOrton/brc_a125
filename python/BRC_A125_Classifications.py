...
for dataFile in dataFiles:
    ...
    logReg = LogisticRegression(solver="liblinear", max_iter=10000, penalty='l2')
    ...
    for estimator in estimators:
        ...
        print(estimator["name"] + ' // ' + dataFile)
        print('AUCROC      = ' + str(round(score,4)))
        print('AUCROC perm = ' + str(np.mean(perm_scores).round(4)) + ' \u00B1 ' + str(np.std(perm_scores).round(4)))
        print('p-value     = ' + str(pvalue.round(5)) + ' = (' + str(pvalue*(n_permutations+1)-1) + ' + 1)/(' + str(n_permutations) + ' + 1)')
